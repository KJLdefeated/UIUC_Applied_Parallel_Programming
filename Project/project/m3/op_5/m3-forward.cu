...
__global__ void matrix_unrolling_kernel(const float *input, half2 *output,
    ...
    */
    ...
    if (row < (size_t) Channel * K * K && col < (size_t) Batch * Height_out * Width_out) {
        output[row * ((size_t) Batch * Height_out * Width_out) + col] = __float2half2_rn(in_4d(b, c, i + i_off, j + j_off));
    }
...
__global__ void matrixMultiplyShared(const float *A, const half2 *B, half2 *C,
                                     int numARows, int numAColumns,
                                     int numBRows, int numBColumns,
                                     int numCRows, int numCColumns)
{
    __shared__ half2 tileA[TILE_WIDTH][TILE_WIDTH];
    __shared__ half2 tileB[TILE_WIDTH][TILE_WIDTH];
...
// You don't need to modify this kernel.
__global__ void matrix_permute_kernel(const half2 *input, float *output, int Map_out,
                                      int Batch, int image_size) {
    int b = blockIdx.y;
    int x = blockIdx.x * BLOCK_SIZE + threadIdx.x;
    if (x < image_size) {
        for (int m = 0; m < Map_out; m++) {
            output[b * Map_out * image_size + m * image_size + x] = __low2float(input[m * Batch * image_size + b * image_size + x]);    
        }
    }
}
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    half2 *unrolled_matrix;  // Pointer to device memory for storing the unrolled matrix
    half2 *matmul_output;    // Pointer to device memory for storing the result of matrix multiplication
    cudaMalloc((void**)&unrolled_matrix, (size_t) Batch * Channel * K * K * Height_out * Width_out * sizeof(half2));
    cudaMalloc((void**)&matmul_output, (Batch * Map_out * Height_out * Width_out) * sizeof(half2));
