...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if (row < dim) {
    int permRow = matRowPerm[row];
    int rowN = matRows[row];
    float dot = 0;
    for (int i = 0; i < rowN; i++) {
      int col_index = matColStart[i] + row;
      dot += matData[col_index] * vec[matCols[col_index]];
    }
    out[permRow] = dot;
  }
}
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  spmvJDSKernel<<<blocksPerGrid, threadsPerBlock>>>(out, matColStart, matCols, matRowPerm, matRows, matData, vec, dim);
}
